...
/**
 * Detect dependencies of the components from `package.json`.
 *
...
function findComponentConfigFile(config, component) {
  ...
  ['package.json', '.package.json', 'component.json', 'package.json'].
...
function findMainFiles(config, component, componentConfigFile) {
  ...
  if ($._.isString(componentConfigFile.main)) {
    // start by looking for what every component should have: config.main
      fileis = $.path.join(config.get('bower-directory'), componentConfigFile.main);
      // console.log(fileis);
      filePaths = [componentConfigFile.main];
    if ($.fs.existsSync(fileis)) {
      filePaths = [componentConfigFile.main];
    } else {
      ['js', 'css', 'scss', 'sass'].forEach(function (type) {
      ...
      ['js', 'css', 'scss', 'sass'].forEach(function (type) {
          file[type] = $.path.join(config.get('bower-directory'), component,type, componentConfigFile.name + '.' + type);
            // console.log(file[type]);
            // console.log($.fs.existsSync(file[type]));
          if ($.fs.existsSync(file[type])) {
            filePaths.push(type+'/'+componentConfigFile.name + '.' + type);
          }
        });
      ['js', 'css', 'scss', 'sass'].forEach(function (type) {
          file[type] = $.path.join(config.get('bower-directory'), component,'dist',type, componentConfigFile.name + '.' + type);
          console.log(file[type]);
          console.log($.fs.existsSync(file[type]));
          if ($.fs.existsSync(file[type])) {
          filePaths.push('dist'+'/'+type+'/'+componentConfigFile.name + '.' + type);
          }
        });
    }
  ...
  } else if ($._.isArray(componentConfigFile.sass)) {
    // still haven't found it. is it stored in config.sass, then?
    filePaths = componentConfigFile.sass;
  } else if ($._.isArray(componentConfigFile.style)) {
    // still haven't found it. is it stored in config.style, then?
    filePaths = componentConfigFile.style;
  } else if ($._.isArray(componentConfigFile.bundlesize)) {
    // still haven't found it. is it stored in config.bundlesize, then?
    filePaths = componentConfigFile.bundlesize;
  } else {
    ['js', 'css', 'scss', 'sass'].forEach(function (type) {
        file[type] = $.path.join(config.get('bower-directory'), component, componentConfigFile.name + '.' + type);
        if ($.fs.existsSync(file[type])) {
...
function gatherInfo(config) {
  ...
  return function (version, component) {
    ...
    if (!componentConfigFile) {
      var error = new Error(component + ' is not installed. Try running `bower install` or remove the component from your package.json file.');
